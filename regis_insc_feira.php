...
<script type="text/javascript">
  ...
  function desativarProfessor(){
      document.getElementById("form-prof1").setAttribute("style", "display: none;");
      document.getElementById("form-prof2").setAttribute("style", "display: none;");
      document.getElementById("professor").setAttribute("disabled", "true");
      document.getElementById("professorR").setAttribute("disabled", "true");
  }

  function ativarProfessor(){
      document.getElementById("form-prof1").removeAttribute("style");
      document.getElementById("form-prof2").removeAttribute("style");
      document.getElementById("professor").removeAttribute("disabled");
      document.getElementById("professorR").removeAttribute("disabled");
  }
    ...
    <h1 class="mb-3 font-weight-normal"> Cadastro e Inscrição - Feira </h1><br/>
      <div style="background-color: #F5F5F5; width: 350px;border-style: solid; border-width: thin; border-color: #C0C0C0; padding: 10px;"><br/>
          <form action="processa_regis_insc_feira.php" method="POST">
            ...
            <div class="form-group custom-radio-final">
                <p class="font-weight-bold">Você é um professor da Trajano Camargo?</p>
                <input type="radio" id="nao" name="escolha" value="0" onclick="desativarProfessor()" required>
                <label for="nao" class="nao">Não</label>
                <input type="radio" id="sim" name="escolha" value="1" onclick="ativarProfessor()" required>
                <label for="sim" class="sim">Sim</label>
            </div><br>
            <div class="form-group" id="form-prof1" style="display: none;">
                <label for="professor"><b>Selecione seu nome:</b></label>
                <select class="form-control" name="professor" id="professor" required disabled>
                    <option value="">...</option>
                    <?php 
            ...
            <div class="form-group" id="form-prof2" style="display: none;">
                <label for="professorR"><b>Selecione novamente:</b></label>
                <select class="form-control" name="professorR" id="professorR" required disabled>
                    <option value="">...</option>
                    <?php 
                    $query_professores = mysqli_query($conn, "SELECT * FROM orientador
                    WHERE id_orientador NOT IN (SELECT id_professor FROM associacao_professor)");
