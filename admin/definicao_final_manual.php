...
if (empty($_GET['id_avaliador'])) {
    ...
    "SELECT i.id_usuario, u.nome FROM inscricao_final AS i
    INNER JOIN usuario as u ON i.id_usuario = u.id_usuario
    ");
?>

    <center>
    <h1 class="font-weight-normal mt-2 mb-2">Avaliação Final</h1>
    ...
    <div style="background-color: #F5F5F5; width: 350px;border-style: solid; border-width: thin; border-color: #C0C0C0;"><br/>
        <form action="definicao_avaliacao.php" method="GET">
            <div class="form-group">
                ...
                <select id="avaliador" name="id_avaliador" style="width: 300px" class="form-control" required>
                    ...
                    while ($avaliador = mysqli_fetch_array($query_avaliador)) {
                        ...
                        if (mysqli_num_rows(mysqli_query($conn, "SELECT id_avaliador FROM avaliacao_final WHERE id_avaliador = $id_avaliador AND status_avaliacao = 0")) > 0) {
            ...
            <input type="text" name="escolha" value="2" required readonly hidden>
...
<?php
    ...
    LEFT JOIN associacao_professor as a ON u.id_usuario = a.id_usuario 
    ...
    WHERE p.id_projeto NOT IN (SELECT id_projeto FROM avaliacao_final WHERE id_avaliador = $id_avaliador AND status_avaliacao = 0) AND id_orientador != $id_professor AND id_coorientador != $id_professor AND p.endereco IS NOT NULL AND p.status_aprovacao = 2 AND ($where_ods) 
    ORDER BY p.id_ods");
    ...
    INNER JOIN ods as o ON p.id_ods = o.id_ods
    INNER JOIN curso as c ON p.id_curso = c.id_curso
    WHERE p.id_projeto NOT IN (SELECT id_projeto FROM avaliacao_final WHERE id_avaliador = $id_avaliador AND status_avaliacao = 0) AND p.id_projeto NOT IN 
    (SELECT p.id_projeto FROM projeto as p 
    WHERE p.endereco IS NOT NULL AND p.status_aprovacao = 2 AND ($where_ods)) AND id_orientador != $id_professor AND id_coorientador != $id_professor AND p.endereco IS NOT NULL AND p.status_aprovacao = 2
    ORDER BY titulo");

    $query_projetos_def = mysqli_query($conn, 
    "SELECT a.*, p.titulo, o.categoria FROM avaliacao_final AS a
...
?>
    ...
    <div class="container">
        <form action="incluir_avaliacao.php" method="POST">
            ...
            <input type="text" name="avaliacao" value="2" hidden readonly>
            <div class="custom-checkbox">
                ...
                if (mysqli_num_rows($query_projeto_ods) > 0) {
                    while ($projeto_ods = mysqli_fetch_array($query_projeto_ods)) {
                        ...
                        $qntd_avaliadores = mysqli_num_rows(mysqli_query($conn, "SELECT id_projeto FROM avaliacao_final WHERE id_projeto = $id_projeto AND status_avaliacao = 0"));
                ...
                if (mysqli_num_rows($query_projetos_def) > 0) {
                    ...
                    ?>
                        <div>
                            ...
                            <a href="excluir_avaliacao.php?id_avaliacao=<?php echo $id_avaliacao ?>&avaliacao=2" class="btn btn-danger" style="width: 100%">Remover</a>
                        </div>
                    <?php
                    }
                }
