...
if (mysqli_num_rows(mysqli_query($conn, "SELECT * FROM usuario WHERE email = '$email'")) > 0) {
    ...
    window.location.href = 'regis_insc_feira.php'
    </script>";
}
else{
    ...
    if (mysqli_query($conn, $sql)) {
        ...
        if ($escolha == 1) {
            ...
            VALUES(
                ...
                echo "<script type='text/javascript'>
                alert('ERRO AO ASSOCIAR PROFESSOR. VOLTE A PÁGINA E TENTE NOVAMENTE.');
                window.location.href = 'regis_insc_feira.php'
                </script>";
                exit();
            }
        ...
        if ($count != 0) {
            if (mysqli_multi_query($conn, $query_ods)) {
                ...
                if (mysqli_query($conn, "INSERT INTO inscricao_final VALUES (NULL, $id_usuario)")) {
                    ...
                    mysqli_query($conn, "DELETE FROM usuario WHERE id_usuario = $id_usuario");
                    echo "<script type='text/javascript'>
                    alert('ERRO AO INSCREVER COM ODS. VOLTE A PÁGINA E TENTE NOVAMENTE.');
                    window.location.href = 'regis_insc_feira.php'
                    </script>";
                ...
                mysqli_query($conn, "DELETE FROM usuario WHERE id_usuario = $id_usuario");
                echo "<script type='text/javascript'>
                    alert('ERRO AO ASSOCIAR ODS. VOLTE A PÁGINA E TENTE NOVAMENTE.');
                    window.location.href = 'regis_insc_feira.php'
                    </script>";
            }
        }
            ...
            if (mysqli_query($conn, "INSERT INTO inscricao_final VALUES (NULL, $id_usuario)")) {
                ...
                mysqli_query($conn, "DELETE FROM usuario WHERE id_usuario = $id_usuario");
                echo "<script type='text/javascript'>
                    alert('ERRO AO INSCREVER SEM ODS. VOLTE A PÁGINA E TENTE NOVAMENTE.');
                    window.location.href = 'regis_insc_feira.php'
                    </script>";
            }
        }
        ...
        echo "<script type='text/javascript'>
        alert('ERRO AO CADASTRAR. VOLTE A PÁGINA E TENTE NOVAMENTE.');
        window.location.href = 'regis_insc_feira.php'
        </script>";
    }    
