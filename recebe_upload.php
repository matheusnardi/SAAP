<?php
    ...
    // O arquivo passou em todas as verificações, hora de tentar movê-lo para a pasta
        ...
        // Verifica se o projeto ainda tem alterações restantes
        // O aluno é redirecionado de volta, caso o projeto não tenha alterações restantes
        if(mysqli_fetch_assoc(mysqli_query($conn, "SELECT endereco_count FROM projeto WHERE id_projeto=$id_projeto;"))['endereco_count'] > 0){
    
            // Depois verifica se é possível mover o arquivo para a pasta escolhida
            if (move_uploaded_file($_FILES['arquivo']['tmp_name'], $_UP['pasta'] . $nome_final)) {

                // Verifica se o projeto já possuí um arquivo
                $antigo_endereco = mysqli_fetch_assoc(mysqli_query($conn, "SELECT endereco FROM projeto WHERE id_projeto=$id_projeto;"))['endereco'];    
                
                // Inserir novo endereco do arquivo no banco de dados
                $novo_endereco = $_UP['pasta'].$nome_final;

                if(mysqli_query($conn, "UPDATE projeto SET endereco = '$novo_endereco', endereco_count = endereco_count-1 WHERE id_projeto=$id_projeto;")){
        ...
        else{ //Se chegar aqui, significa que o projeto não tem alterações restantes, então ele volta a página para que não seja upado o arquivo
            mysqli_close($conn);
            echo "<script type='text/javascript'>
                            alert('O projeto não possuí alterações restantes!');
                            window.location.href = history.back()
                            </script>";
        }
    }   
